...
struct Line_Point Task_1_Put_Point_Offset[5] = {
	...
	{{0, -200}, 0, 40, 100, 40, -1, 2},//green
	{{167, 0}, 0, 40, 100, 40, -1, 2},//red
	{{0, 200}, 0, 40, 100, 40, -1, 2},//white
...
//	u8 is_key = 0;	
	GPS_Clear();
	Show_Keyboard();
	Return_Any_Point(&Center_Point,10.0,0.5);  // Center Point
	delay_ms(1000);
	...
	for(;pos_Grab_cnt <= 4;)
	{
		set_loc[0] = 400000;
		if(pos_Grab_cnt <= 2)
			set_loc[1] = PanTilt_Zero + 1024*(2*pos_Grab_cnt - 1);// 1_point +50 2_point +50 3_point +25 4_point +25
		else if(pos_Grab_cnt > 2 && pos_Grab_cnt <= 4)
			set_loc[1] = PanTilt_Zero + 1024*(2*pos_Grab_cnt - 1);
		...
		Return_Any_Point(&put_line_point,5.0,0.5);
		delay_ms(500);
