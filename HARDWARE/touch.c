...
void Input_Int64Value(int64_t *address,char *name)//长整型输入 longlong
{
	u8 tp_last, key_value, is_key = 0;
	u8 pre_KeyBoard_State = KeyBoard_State;
	...
	Show_Keyboard();
	LCD_ShowString(0,6,300,24,24,(u8*)name);
	LCD_printf(0,6+36,300,24,24,"old:%lld",*address);
	LCD_printf(0,6+2*36,300,24,24,"new:%s",str);
	for(;;)
	{		
		#ifdef BSP_USING_TOUCH_SCREEN
...
u8 keyScan(u8 menuType)
{
	...
	if ((tp_dev.sta&TP_PRES_DOWN) && !tp_last)
	{
		...
		{
			keyValue = TP_MainMenu_Judge(tp_dev.x[0], tp_dev.y[0]);
		}
		else if(menuType == KEYBOARD)
		{
			keyValue = TP_Keyboard_Judge(tp_dev.x[0], tp_dev.y[0]);
		}
		else
		{
			keyValue = TP_Row_Judge(tp_dev.x[0], tp_dev.y[0]);			
		}
		if (keyValue != keyempty1)
			is_key = 1;
	}
	#endif
	#ifdef BSP_USING_USART_KEY
